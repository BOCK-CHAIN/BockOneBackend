...
async function main() {
  ...
  const customerHashed = await bcrypt.hash('TestPass123!', 10);
  ...
  await prisma.menuItem.createMany({
    data: [
      { name: 'Margherita Pizza', description: 'Classic cheese and tomato', price: 12.99, restaurantId: pizzaPalace.id },
      { name: 'Pepperoni Pizza', description: 'Pepperoni with mozzarella', price: 14.99, restaurantId: pizzaPalace.id },
      { name: 'BBQ Chicken Pizza', description: 'Grilled chicken with BBQ sauce', price: 16.99, restaurantId: pizzaPalace.id },
      { name: 'Garlic Bread', description: 'Toasted bread with garlic butter', price: 4.99, restaurantId: pizzaPalace.id },
    ],
  });
  ...
  await prisma.menuItem.createMany({
    data: [
      { name: 'Classic Burger', description: 'Beef patty with lettuce and tomato', price: 8.99, restaurantId: burgerBarn.id },
      { name: 'Cheese Burger', description: 'Classic with cheddar cheese', price: 9.99, restaurantId: burgerBarn.id },
      { name: 'Bacon Burger', description: 'Loaded with crispy bacon', price: 11.99, restaurantId: burgerBarn.id },
      { name: 'French Fries', description: 'Crispy golden fries', price: 3.99, restaurantId: burgerBarn.id },
    ],
  });
