...
// 1. Enable CORS for all origins (open for BockFoods)
const corsOptions = {
...
// Health check
app.get('/api/foods/health', (req, res) => {
  res.json({ status: 'ok', service: 'bockfoods-backend', timestamp: new Date().toISOString() });
});

app.use('/api/foods/api/auth', authRouter);
app.use('/api/foods/api/users', userRouter);
app.use('/api/foods/api/restaurants', restaurantRouter);
app.use('/api/foods/api/menu-items', menuRouter);
app.use('/api/foods/api/cart', cartRouter);
app.use('/api/foods/api/orders', orderRouter);
app.use('/api/foods/api/grocerystores', groceryRouter);
app.use('/api/foods/api/categories', categoryRouter);
