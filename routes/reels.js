...
function mapReel(reel, profile, isLiked = false) {
  return {
    id: reel.id,
...
router.get('/', optionalJWT, async (req, res) => {
  try {
    ...
    const profiles = await getProfilesByIds((reels || []).map((reel) => reel.user_id));
    const processedReels = (reels || []).map((reel) =>
      mapReel(reel, profiles.get(reel.user_id), false),
    );
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      reel: mapReel(responseData, profile, false),
    });
  } catch (error) {
...
router.get('/user/:id', optionalJWT, async (req, res) => {
  try {
    ...
    const query = supabase.from('reels').select('*').eq('user_id', id);
