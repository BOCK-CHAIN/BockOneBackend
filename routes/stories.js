...
function mapStory(story, profile) {
  // Support both new (media_url/media_type) and legacy (image_url/video_url) column names
  const isVideo = (story.media_type || (story.video_url ? 'video' : null)) === 'video';
  const imageUrl = story.image_url || (isVideo ? null : (story.media_url || null));
  const videoUrl = story.video_url || (isVideo ? (story.media_url || null) : null);
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const publicUrl = supabase.storage.from(storiesBucket).getPublicUrl(storagePath).data.publicUrl;
    ...
    return res.status(201).json({ story: mapStory(story, profile) });
  } catch (error) {
...
router.get('/following', requireAuth, async (req, res) => {
  try {
    ...
    const profiles = await getProfilesByIds((stories || []).map((story) => story.user_id));
    const grouped = {};
    for (const s of stories || []) {
      const uid = s.user_id;
      if (!grouped[uid]) grouped[uid] = [];
      grouped[uid].push(mapStory(s, profiles.get(uid)));
    }
...
router.get('/user/:id', async (req, res) => {
  try {
    ...
    const profiles = await getProfilesByIds((stories || []).map((story) => story.user_id));
    return res.json({
      stories: (stories || []).map((story) => mapStory(story, profiles.get(story.user_id))),
    });
  } catch (error) {
