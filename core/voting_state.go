...
func (vs *VotingState) ApproveVoter(voterID string, adminKey crypto.PublicKey) error {
	...
	// Verify admin key against at least one election's admin key
	authorized := false
	...
	if !authorized {
		return fmt.Errorf("unauthorized: only an election admin can approve voters")
	}
...
func (vs *VotingState) RejectVoter(voterID string, adminKey crypto.PublicKey) error {
	...
	if !authorized {
		return fmt.Errorf("unauthorized: only an election admin can reject voters")
	}
...
// FindVoterByPublicKey finds a voter by their public key
func (vs *VotingState) FindVoterByPublicKey(pubKey crypto.PublicKey) (*Voter, string, error) {
	vs.mu.RLock()
	defer vs.mu.RUnlock()

	pubKeyStr := pubKey.String()
	for id, voter := range vs.voters {
		if voter.PublicKey.String() == pubKeyStr {
			return voter, id, nil
		}
	}

	return nil, "", fmt.Errorf("voter not found for public key")
}

// HasVoterVoted checks if a voter has already voted in an election
func (vs *VotingState) HasVoterVoted(electionID, voterID string) bool {
	vs.mu.RLock()
	defer vs.mu.RUnlock()

	if vs.hasVoted[electionID] == nil {
		return false
	}
	return vs.hasVoted[electionID][voterID]
}

// GetVoterVotes returns all election IDs a voter has voted in
func (vs *VotingState) GetVoterVotes(voterID string) []string {
	vs.mu.RLock()
	defer vs.mu.RUnlock()

	var votes []string
	for electionID, voters := range vs.hasVoted {
		if voters[voterID] {
			votes = append(votes, electionID)
		}
	}
	return votes
}
