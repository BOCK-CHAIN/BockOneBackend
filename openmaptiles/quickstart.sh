...
if [ $# -eq 0 ]; then
  # default to Geofabrik Southern Zone extract (India)
  export area=asia/india/southern-zone
  osm_server=geofabrik
  echo "No parameter - set area=$area (server=$osm_server)"
else
  export area=$1
fi
...
# keep Geofabrik as default source unless explicitly overridden
: "${osm_server:=geofabrik}"
