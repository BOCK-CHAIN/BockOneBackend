    ...
    try {
        ...
        if (err.code === '42P01') {
            return res.status(500).json({
                error: 'Database is not initialized. Run `npm run setup-db` in EiraFlutterBackend.',
            });
        }
        if (err.code === 'ECONNREFUSED' || err.code === '28P01' || err.code === '3D000') {
            return res.status(500).json({
                error: 'Database connection failed. Check DATABASE_URL in EiraFlutterBackend/.env.',
            });
        }
