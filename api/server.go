...
func (s *Server) Start() error {
	...
	e.GET("/api/vote/health", func(c echo.Context) error {
		return c.JSON(http.StatusOK, map[string]string{
			"status": "ok",
			"service": "bockvote-backend",
		})
	})

	e.GET("/api/vote/block/:hashorid", s.handleGetBlock)
	e.GET("/api/vote/tx/:hash", s.handleGetTx)
	e.POST("/api/vote/tx", s.handlePostTx)

	// Voting API endpoints
	e.GET("/api/vote/voting/elections", s.handleListElections)
	e.POST("/api/vote/voting/register/voter", s.handleRegisterVoter)
	e.POST("/api/vote/voting/register/candidate", s.handleRegisterCandidate)
	e.POST("/api/vote/voting/election/create", s.handleCreateElection)
	e.POST("/api/vote/voting/vote", s.handleCastVote)
	e.GET("/api/vote/voting/election/:id", s.handleGetElection)
	e.GET("/api/vote/voting/election/:id/results", s.handleGetElectionResults)
	e.GET("/api/vote/voting/election/:id/voter/:voterId/has-voted", s.handleCheckVoterVoted)
	e.GET("/api/vote/voting/voter/:id", s.handleGetVoter)
	e.GET("/api/vote/voting/voter/:id/votes", s.handleGetVoterVotes)
	e.GET("/api/vote/voting/voter/find-by-key", s.handleFindVoterByKey)
	e.GET("/api/vote/voting/candidate/:electionId/:id", s.handleGetCandidate)
	e.POST("/api/vote/voting/approve/voter", s.handleApproveVoter)
	e.POST("/api/vote/voting/approve/candidate", s.handleApproveCandidate)
...
// handleCastVote handles vote casting requests with upfront validation
func (s *Server) handleCastVote(c echo.Context) error {
	...
	// Synchronously validate the vote before queueing
	vs := s.bc.GetVotingState()

	// Check election exists and is active
	now := time.Now().Unix()
	election, err := vs.GetElection(req.ElectionID)
	if err != nil {
		return c.JSON(http.StatusBadRequest, APIError{Error: err.Error()})
	}
	if now < election.StartTime {
		return c.JSON(http.StatusBadRequest, APIError{Error: "election has not started yet"})
	}
	if now >= election.EndTime {
		return c.JSON(http.StatusBadRequest, APIError{Error: "election has ended"})
	}

	// Find voter and check approval
	voter, voterID, err := vs.FindVoterByPublicKey(pubKey)
	if err != nil {
		return c.JSON(http.StatusBadRequest, APIError{Error: "voter not found or not registered"})
	}
	if voter.Status != core.VoterStatusApproved {
		return c.JSON(http.StatusBadRequest, APIError{Error: "voter is not approved to vote"})
	}

	// Check candidate exists and is approved
	candidate, err := vs.GetCandidate(req.ElectionID, req.CandidateID)
	if err != nil {
		return c.JSON(http.StatusBadRequest, APIError{Error: err.Error()})
	}
	if candidate.Status != core.CandidateStatusApproved {
		return c.JSON(http.StatusBadRequest, APIError{Error: "candidate is not approved to receive votes"})
	}

	// Check if voter already voted
	if vs.HasVoterVoted(req.ElectionID, voterID) {
		return c.JSON(http.StatusConflict, APIError{Error: "voter has already cast a vote in this election"})
	}
...
// handleCheckVoterVoted checks if a voter has voted in an election
func (s *Server) handleCheckVoterVoted(c echo.Context) error {
	electionID := c.Param("id")
	voterID := c.Param("voterId")
	if electionID == "" || voterID == "" {
		return c.JSON(http.StatusBadRequest, APIError{Error: "election ID and voter ID are required"})
	}

	hasVoted := s.bc.GetVotingState().HasVoterVoted(electionID, voterID)
	return c.JSON(http.StatusOK, map[string]interface{}{
		"hasVoted": hasVoted,
	})
}

// handleGetVoterVotes returns all elections a voter has voted in
func (s *Server) handleGetVoterVotes(c echo.Context) error {
	voterID := c.Param("id")
	if voterID == "" {
		return c.JSON(http.StatusBadRequest, APIError{Error: "voter ID is required"})
	}

	votes := s.bc.GetVotingState().GetVoterVotes(voterID)
	return c.JSON(http.StatusOK, map[string]interface{}{
		"votes": votes,
	})
}

// handleFindVoterByKey finds a voter by their public key (via X-Private-Key header)
func (s *Server) handleFindVoterByKey(c echo.Context) error {
	privKey, err := parseRequestPrivateKey(c.Request().Header.Get("X-Private-Key"))
	if err != nil {
		return c.JSON(http.StatusBadRequest, APIError{Error: err.Error()})
	}

	voter, voterID, err := s.bc.GetVotingState().FindVoterByPublicKey(privKey.PublicKey())
	if err != nil {
		return c.JSON(http.StatusNotFound, APIError{Error: err.Error()})
	}
	...
	return c.JSON(http.StatusOK, VoterResponse{
		ID:          voterID,
		PublicKey:   voter.PublicKey.String(),
		IPFSDocHash: voter.IPFSDocHash,
		Status:      statusStr,
